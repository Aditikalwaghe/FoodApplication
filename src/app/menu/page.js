...
export default function Menu() {
  ...
  // ✅ Protect Remove From Cart (Optional but Recommended)
  ...
  const toggleWishlist = (foodId) => {
    ...
    if (userWishlist.includes(foodId)) {
      updatedWishlist = userWishlist.filter((id) => id !== foodId);
    } else {
      updatedWishlist = [...userWishlist, foodId];
    }

    allWishlists[currentUser] = updatedWishlist;
    localStorage.setItem("wishlists", JSON.stringify(allWishlists));
    setWishlist(updatedWishlist); // Update React state to reflect immediately
  };
  ...
  const openReviews = (foodId) => {
    ...
    const foodReviews = allReviews.filter((r) => r.foodId === foodId);
